...
import { useAdAdminNoticeQuery } from "../query";
...
const NoticeTable = ({keyword, type}) => {
  ...
  const { adminNoticeData, isLoading } = useAdAdminNoticeQuery(paramsPage, keywordValue, ...type);
  const [current, setCurrent] = useState(adminNoticeData?.data.currentPage + 1);
  ...
  useEffect(() => {
    setCurrent(adminNoticeData?.data.currentPage + 1);
  ...
  return ( 
    <div className="w-full pt-[20px] pb-[46px]">
            ...
            <div className="flex flex-col items-end justify-center gap-[6px]">
              <div className="font-nanum_700 text-[13px] flex items-center justify-end gap-[6px]">
                {item.adminNoticeType.map((type, i) => (
                  <div key={i}>
                    {type == "STUDENT" ? (<div className="text-main1">학생</div>)
                      : type == "PARENT" ? (<div className="text-main2">학부모</div>)
                        : (<div className="text-main3">선생님</div>)
                    }
                  </div>
                ))}
              </div>
              <div className="font-nanum_400 text-[13px] text-grey">{item.created.slice(0,10)}</div>
            </div>
          </div>
      ...
      <div className="flex justify-center items-center w-full tablet:gap-[60px] mobile:gap-[30px] text-[12px] mt-[12px]">
        ...
        <button onClick={NextPage} disabled={current === adminNoticeData?.data.totalPage} className={current === adminNoticeData?.data.totalPage ? 'invisible' : ''}>
          <div className="flex justify-center items-center gap-[3px]">
