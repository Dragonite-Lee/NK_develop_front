...
import { useAdOneClassroomQuery } from "../query";
...
const BlueModalClPut = ({setState, data}) => {
  ...
  useEffect(() => {
    if (oneClassroomData) {
      ...
      const coTeacherName = oneClassroomData.data.AssistantTeacher.map((item)=>item.nickname);
      const coTeacherId = oneClassroomData.data.AssistantTeacher.map((item)=>item.id);
      let coTeacherPutId= [];
  ...
  // console.log(oneClassroomData.data)
    ...
    <div className="z-10 fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full y-full min-h-full bg-[#212121]/[.08]">
      <div className="bg-white fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] tablet_change:w-[694px] tablet:w-[383px] mobile:w-[260px] tablet_change:h-[265px] mobile:h-[381px] rounded-[20px] border drop-shadow">
        ...
        <div className="flex items-center justify-center gap-[16px] mt-[24px] font-nanum_400 text-[14px]">
          <button onClick={() => putMutate.mutate(data, classroomPostData)} className="bg-management1 text-white  tablet_change:w-[109px] mobile:w-[99px] h-[40px] flex items-center justify-center rounded-[10px]">수정하기</button>
