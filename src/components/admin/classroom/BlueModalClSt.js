...
import { deleteAdClassroomStudent, postAdTeacher } from "../../../services/api/adminApi";
import { useAdClassroomStudentQuery } from "../query";
...
const BlueModalClSt = ({setState, id, title}) => {
  ...
  const deleteMutate = useMutation({
    mutationFn: () => {
      return deleteAdClassroomStudent(id, cancleId);
    },
    onSuccess: () => {
    ...
    <div className="z-10 fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full y-full min-h-full bg-[#212121]/[.08]">
      <div className="bg-white fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] tablet_change:w-[694px] tablet:w-[383px] mobile:w-[260px] h-[430px] rounded-[20px] border drop-shadow">
        <div className="h-[64px] bg-management1 flex items-center tablet:pl-[36px] mobile:px-[12px] rounded-t-[20px]">
          <div className="font-pacbooc_700 text-[18px] text-white">{title} 학생 명단 관리</div>
        </div>
        <div className="tablet:text-[14px] mobile:text-[12px] font-nanum_700">
          {cancleModal && (
            <YellowModalCl setState={setCancleModal} id={id} mutate={deleteMutate.mutate} selection={cancleId} title={`학생 삭제`} content1={`선택한 학생을 삭제하시겠습니까?`} content2="" content3="" cancle="취소하기" del="삭제하기" />
          )}
          {plusModal && (
            <BlueModalPlusSt setState={setPlusModal} id={id} />
          )}
          {/* 테이블 */}
          {classroomStudentData?.data.length == 0 ? (
            <div className="font-nanum_700 text-[14px] mt-[24px] px-[16px]">등록된 데이터가 존재하지 않습니다.</div>
          ) : (
        ...
        <div className="flex items-center justify-center gap-[16px] mt-[24px] font-nanum_400 text-[14px]">
          <button onClick={() => setPlusModal(true)} className="bg-management1 text-white  tablet_change:w-[109px] mobile:w-[99px] h-[40px] flex items-center justify-center rounded-[10px]">학생 추가</button>
          <button onClick={() => setState(false)} className="bg-grayLight text-white tablet_change:w-[109px] mobile:w-[99px] h-[40px] flex items-center justify-center rounded-[10px]">닫기</button>
        </div>
      </div>
    </div>
  );
