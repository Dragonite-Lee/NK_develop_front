...
const BlueModalPlusSt = ({setState, id}) => {
  ...
  const onChangeAllSelection = (e) => {
    if (e.target.checked) {
      const allCheckedSelection = new Set(allStudentData?.data.map((data) => data.id));
      ...
      <div className="z-10 fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full y-full min-h-full bg-[#212121]/[.08]">
        <div className="bg-white fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] tablet_change:w-[694px] tablet:w-[383px] mobile:w-[260px] h-[430px] rounded-[20px] border drop-shadow">
          <div className="h-[64px] bg-management1 flex items-center pl-[36px] rounded-t-[20px]">
            <div className="font-pacbooc_700 text-[18px] text-white">학생 추가</div>
          </div>
          {/* 학교 등록 */}
          <div className="mt-[6px] flex tablet_change:flex-row mobile:flex-col gap-[8px] tablet_change:items-center tablet_change:justify-end mobile:items-end p-[12px]">
            <div className="relative tablet_change:w-[300px] mobile:w-[200px] h-[36px]">
              <input type="search" placeholder="학생을 검색하세요" value={searchInput} onChange={setSearchInput} className="tablet_change:w-[300px] mobile:w-[200px] h-[36px] text-[14px] font-nanum_400 pl-[15px] rounded-[10px] bg-whiteTotal drop-shadow-sm" />
              {!searchInput && (
          ...
          {/* 학교 테이블 */}
          <table className="px-[12px] mt-[6px] text-[14px] grid tablet_change:w-[692px] overflow-auto scrollbar-thin scrollbar-webkit	scroll-behavior:smooth tablet_change:pb-[49px] mobile:pb-[29px]">
            ...
            <tbody className="font-nanum_400 px-[16px] w-[657px] h-[120px]">
              {
          ...
          <div className="flex items-center justify-center gap-[16px] mt-[18px] font-nanum_400 text-[14px]">
            <button onClick={() => postMutate.mutate(id, studentPostData)} className="bg-management1 text-white  tablet_change:w-[109px] mobile:w-[99px] h-[40px] flex items-center justify-center rounded-[10px]">학생 추가</button>
