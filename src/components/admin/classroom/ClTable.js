...
const ClTable = ({cancleText, cancleText2, keyword, header, updateSelection, deleteMutate}) => {
  ...
  const onChangeAllSelection = (e) => {
    if (e.target.checked) {
      const allCheckedSelection = new Set(classroomData?.data.results.map((data) => data.id));
  ...
  useEffect(() => {
    ...
    setCurrent(classroomData?.data.currentPage + 1)
  ...
  return ( 
    <div className="mt-[12px]">
            ...
            classroomData?.data.results.map((data, index) => (
              <tr key={index} className="flex items-center justify-between">
                ...
                <td className="flex items-center justify-between gap-[12px]">
                  <img src={Pencil} alt="pencil" className="w-[20px] h-[20px]" onClick={()=>onClickModifyModal(data.id)} />
      ...
      <div className="flex justify-center items-center w-full tablet:gap-[60px] mobile:gap-[30px] text-[12px] mt-[12px]">
        ...
        <button onClick={NextPage} disabled={current === classroomData?.data.totalPage} className={current === classroomData?.data.totalPage ? 'invisible' : ''}>
          <div className="flex justify-center items-center gap-[3px]">
