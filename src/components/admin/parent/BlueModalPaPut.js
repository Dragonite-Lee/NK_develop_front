...
import { useAdOneParentQuery } from "../query";
...
const BlueModalPaPut = ({setState, data}) => {
  ...
  const [relationshipValue, setRelationship] = useState("");
  ...
  useEffect(() => {
    if (oneParentData) {
      setNickname(oneParentData.data.nickname);
      setRelationship(oneParentData.data.relationship)
      ...
      const allStudentNicknames = oneParentData.data.students.map((data) => data.nickname);
      setStudentIds(allStudentIds);
      setStudentIdsValue(allStudentNicknames);
      // console.log(allStudentIds, allStudentNicknames)
    }
  }, [oneParentData])
    ...
    <div className="z-10 fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full y-full min-h-full bg-[#212121]/[.08]">
      <div className="bg-white fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] tablet_change:w-[694px] tablet:w-[383px] mobile:w-[260px] tablet_change:h-[323px] mobile:h-[484px] rounded-[20px] border drop-shadow">
        <div className="h-[64px] bg-management1 flex items-center pl-[36px] rounded-t-[20px]">
          <div className="font-pacbooc_700 text-[18px] text-white">학부모 수정</div>
        </div>
        <div className="tablet:text-[14px] mobile:text-[12px] font-nanum_700 flex tablet_change:flex-row flex-col items-start justify-between tabelt_change:gap-[30px] mobile:gap-[17px] mt-[24px] tablet:px-[40px] mobile:px-[20px] ">
          <div className="flex flex-col gap-[17px] tablet_change:w-[292px] tablet:w-[303px] mobile:w-[220px]">
            ...
            <div className="relative z-20 flex items-center justify-between">
              ...
              <DropdownMenu state={relationshipValue} setState={setRelationship} itemData={relationList} />
            </div>
