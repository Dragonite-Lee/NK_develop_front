...
import { useAdParentQuery } from "../adminQuery";
...
const PaTable = ({
  ...
  const [current, setCurrent] = useState(parentData?.data.currentPage);
  ...
  const onChangeAllSelection = (e) => {
    if (e.target.checked) {
      const allCheckedSelection = new Set(
        parentData?.data.results.map((data) => data.id)
      );
  ...
  useEffect(() => {
    ...
    setCurrent(parentData?.data.currentPage);
  ...
  return (
    <div className="mt-[12px]">
            ...
            <tr key={index} className="flex items-center justify-between">
              ...
              <td className="flex items-center justify-between gap-[12px]">
                <img
                  ...
                  onClick={() => onClickModifyModal(data.username)}
                />
                <img
                  src={Trash}
                  alt="trash"
      ...
      <div className="flex justify-center items-center w-full tablet:gap-[60px] mobile:gap-[30px] text-[12px] mt-[12px]">
        ...
        <button
          onClick={NextPage}
          disabled={current === parentData?.data.totalPage}
          className={current === parentData?.data.totalPage ? "invisible" : ""}
        >
          <div className="flex justify-center items-center gap-[3px]">
