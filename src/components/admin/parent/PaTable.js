...
import BlueModalPaPut from "./BlueModalPaPut";
import { useAdParentQuery } from "../query";
...
const PaTable = ({cancleText, cancleText2, keyword, header, updateSelection, deleteMutate}) => {
  ...
  const { parentData, isLoading } = useAdParentQuery(paramsPage, keyword);
  const [current, setCurrent] = useState(parentData?.data.currentPage + 1);
  ...
  const onChangeAllSelection = (e) => {
    if (e.target.checked) {
      const allCheckedSelection = new Set(parentData?.data.results.map((data) => data.id));
  ...
  const isSelectedAll = () => {
    return selection.size === parentData?.data.results?.length;
  };
  ...
  useEffect(() => {
    ...
    setCurrent(parentData?.data.currentPage + 1)
  },[deleteMutate.isSuccess, parentData?.data])
  ...
  if (parentData?.data.results.length == 0) return <div className="font-nanum_700 text-[14px]">등록된 데이터가 존재하지 않습니다.</div>
  ...
  return ( 
    <div className="mt-[12px]">
      ...
      {modifyModal && (
        <BlueModalPaPut setState={setModifyModal} data={modifyId}/>
      )}
      {/* 테이블 */}
      <table className="mt-[20px] text-[14px] grid desktop:w-[996px] overflow-x-auto scrollbar-thin scrollbar-webkit	scroll-behavior:smooth tablet_change:pb-[49px] mobile:pb-[29px]">
        ...
        <tbody className="font-nanum_400 px-[16px]">
          {
            parentData?.data.results.map((data, index) => (
                  ...
                  </div>
                  {
                    headerkey.map((key, id) =>
                      <div key={key + id} className={`flex items-center justify-center ${width[id]}`}>
                        <div>{key == 'relationship' ? data[key] == 'FATHER' ? '부' : '모'
                        : key == 'students' ? data[key]?.map((data, index) => (<div key={index}>{data.nickname}</div>))
                          : data[key]}</div>
                      </div>
                    )
                  }
      ...
      <div className="flex justify-center items-center w-full tablet:gap-[60px] mobile:gap-[30px] text-[12px] mt-[12px]">
        ...
        <div className="items-center text-grayDark font-nanum_400">
          {current} / {parentData?.data.totalPage}
        </div>

        <button onClick={NextPage} disabled={current === parentData?.data.totalPage} className={current === parentData?.data.totalPage ? 'invisible' : ''}>
          <div className="flex justify-center items-center gap-[3px]">
