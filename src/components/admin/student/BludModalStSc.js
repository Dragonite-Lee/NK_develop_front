      ...
      <div className="z-10 fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full y-full min-h-full bg-[#212121]/[.08]">
        <div className="bg-white fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] tablet_change:w-[694px] tablet:w-[383px] mobile:w-[260px] tablet_change:h-[373px] mobile:h-[534px] rounded-[20px] border drop-shadow">
          <div className="h-[64px] bg-management1 flex items-center pl-[36px] rounded-t-[20px]">
            <div className="font-pacbooc_700 text-[18px] text-white">학교 관리</div>
          </div>
          {/* 학교 등록 */}
          <div className="mt-[13px] flex tablet_change:flex-row mobile:flex-col gap-[8px] tablet_change:items-center tablet_change:justify-between mobile:items-end p-[12px]">
            <div className="relative tablet_change:w-[300px] mobile:w-[200px] h-[36px]">
              <input type="search" placeholder="학교를 검색하세요" value={searchInput} onChange={setSearchInput} className="tablet_change:w-[300px] mobile:w-[200px] h-[36px] text-[14px] font-nanum_400 pl-[15px] rounded-[10px] bg-whiteTotal drop-shadow-sm" />
              {!searchInput && (
                <img src={MagnifyingGlass} alt="돋보기" className="absolute top-[7px] right-[15px] w-[22px] h-[22px]" />
              )}
            </div>
            <div className="relative tablet_change:w-[300px] mobile:w-[200px] h-[36px]">
              <input type="text" placeholder="학교를 등록하세요" value={registerInput} onChange={setRegisterInput} className="tablet_change:w-[300px] mobile:w-[200px] h-[36px] text-[14px] font-nanum_400 pl-[15px] rounded-[10px] bg-whiteTotal drop-shadow-sm" />
              <button onClick={() => postMutate.mutate(registerData)} className="absolute right-0 bg-management2 rounded-r-[10px] text-white w-[50px] h-[36px] text-[14px] font-nanum_400">등록</button>
            </div>
          </div>
          
          {/* 학교 테이블 */}
          <ScTable cancleText="학교" cancleText2="학교를" header={header} data={searchData} updateSelection={setSelection} onDelete={deleteMutate.mutate} isSuccess={deleteMutate.isSuccess} isLoading={deleteMutate.isLoading}/>
          <div className="flex items-center justify-center gap-[16px] mt-[18px] font-nanum_400 text-[14px]">
            <button onClick={() => setCancleModal(true)} className="bg-management1 text-white  tablet_change:w-[109px] mobile:w-[99px] h-[40px] flex items-center justify-center rounded-[10px]">삭제하기</button>
