...
import { useAdSchoolQuery } from "../query";
...
const BlueModalSt = ({setState}) => {
  ...
  const [birthValue, birthValueHandler] = useState("");
  ...
  const [createDateValue, createDateHandler] = useState("");
  ...
  // useEffect(() => {
  //   if (idValid, pwValid, phoneValid, nicknameValue, schoolNameValue, birthValue, usernameValue, passwordValue, createDateValue, gradeValue) {
  //     setAllCheck(true)
  //   } else {
  //     setAllCheck(false)
  //   }
  // },[idValid, pwValid, phoneValid, nicknameValue, schoolNameValue, birthValue, usernameValue, passwordValue, createDateValue, gradeValue])
  ...
  // console.log(birthValue, usernameValue)
    ...
    <div className="z-10  fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full y-full min-h-full bg-[#212121]/[.08]">
      <div className="bg-white fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] tablet_change:w-[694px] tablet:w-[383px] mobile:w-[260px] tablet_change:h-[373px] mobile:h-[564px] rounded-[20px] border drop-shadow">
        <div className="h-[64px] bg-management1 flex items-center pl-[36px] rounded-t-[20px]">
          <div className="font-pacbooc_700 text-[18px] text-white">학생 신규 등록</div>
        </div>
        <div className="tablet:text-[14px] mobile:text-[12px] font-nanum_700 flex tablet_change:flex-row flex-col items-start justify-between tabelt_change:gap-[30px] mobile:gap-[17px] tablet_change:mt-[24px] mobile:mt-[12px] tablet:px-[40px] mobile:px-[20px] ">
          <div className="flex flex-col gap-[17px] tablet_change:w-[292px] tablet:w-[303px] mobile:w-[220px]">
            <div className="flex items-center justify-between">
              <div>학생 이름 <span className="text-error">*</span></div>
            ...
            <div>
              <div className="flex items-center justify-between">
                <div>학생 아이디 <span className="text-error">*</span></div>
            ...
            <div>
              <div className="flex items-center justify-between">
                <div>학생 비밀번호 <span className="text-error">*</span></div>
            ...
            <div>
              <div className="flex items-center justify-between">
                <div>학생 연락처 <span className="text-error">*</span></div>
          ...
          <div className="flex flex-col gap-[17px] tablet_change:w-[292px] tablet:w-[303px] mobile:w-[220px] tablet:mt-0 mobile:mt-1">
            <div className="relative z-20 flex items-center justify-between">
              <div>재학 중인 학교 <span className="text-error">*</span></div>
              ...
              <SearchDropdownSc state={schoolNameValue} setState={setSchoolName} itemData={schoolData} />
            </div>
            <div className="relative z-10 flex items-center justify-between">
              <div>학년 <span className="text-error">*</span></div>
              <DropdownMenu state={gradeValue} setState={setGradeValue} itemData={gradeList} />
            </div>
            <div className="flex items-center justify-between">
              <div>생년월일 <span className="text-error">*</span></div>
              <input type="date" value={birthValue} onChange={birthValueHandler} className="font-nanum_400 tablet:text-[14px] mobile:text-[12px] tablet:w-[167px] mobile:w-[127px] h-[36px] border-border rounded-[10px] placeholder-grey"/>
            </div>
            <div className="flex items-center justify-between">
              <div>등록일 <span className="text-error">*</span></div>
              <input type="date" value={createDateValue} onChange={createDateHandler} className="font-nanum_400 tablet:text-[14px] mobile:text-[12px] tablet:w-[167px] mobile:w-[127px] h-[36px] border-border rounded-[10px] placeholder-grey"/>
            </div>
          </div>
        </div>
        <div className="flex items-center justify-center gap-[16px] tablet:mt-[24px] mobile:mt-[12px] font-nanum_400 text-[14px]">{/*${allCheck ? ' bg-management1' : ' bg-grayLight'}*/}
          <button onClick={() => postMutate.mutate(studentPostData)} className={`bg-management1 text-white  tablet_change:w-[109px] mobile:w-[99px] h-[40px] flex items-center justify-center rounded-[10px]`}>등록하기</button>
