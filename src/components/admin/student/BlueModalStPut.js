...
import { useAdOneStudentQuery, useAdSchoolQuery } from "../query";
...
const BlueModalStPut = ({setState, data}) => {
  ...
  const { oneStudentData } = useAdOneStudentQuery(data);
  const queryClient = useQueryClient();
  
  const putMutate = useMutation({
    mutationFn: () => {
      return putAdStudent(data, studentPutData);
    },
    onSuccess: () => {
  ...
  useEffect(() => {
    if (oneStudentData) {
      setNickname(oneStudentData.data.nickname);
      setSchoolName(oneStudentData.data.schoolName);
      setBirth(oneStudentData.data.birth)
      setPhoneNumber(oneStudentData.data.phoneNumber);
      setUsername(oneStudentData.data.username)
      setCreateDate(oneStudentData.data.registrationDate)
      setGradeValue(oneStudentData.data.grade)
    }
  }, [oneStudentData])
    ...
    <div className="z-10 fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-full y-full min-h-full bg-[#212121]/[.08]">
      <div className="bg-white fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] tablet_change:w-[694px] tablet:w-[383px] mobile:w-[260px] tablet_change:h-[373px] mobile:h-[564px] rounded-[20px] border drop-shadow">
        <div className="h-[64px] bg-management1 flex items-center pl-[36px] rounded-t-[20px]">
          <div className="font-pacbooc_700 text-[18px] text-white">학생 수정</div>
        </div>
        <div className="tablet:text-[14px] mobile:text-[12px] font-nanum_700 flex tablet_change:flex-row flex-col items-start justify-between tabelt_change:gap-[30px] mobile:gap-[17px] tablet_change:mt-[24px] mobile:mt-[12px] tablet:px-[40px] mobile:px-[20px] ">
          ...
          <div className="flex flex-col gap-[17px] tablet_change:w-[292px] tablet:w-[303px] mobile:w-[220px] tablet:mt-0 mobile:mt-1">
            <div className="relative z-20 flex items-center justify-between">
              ...
              <SearchDropdownMenu state={schoolNameValue} setState={setSchoolName} itemData={schoolData} />
            </div>
        ...
        <div className="flex items-center justify-center gap-[16px] tablet:mt-[24px] mobile:mt-[12px] font-nanum_400 text-[14px]">
          <button onClick={() => putMutate.mutate()} className="bg-management1 text-white  tablet_change:w-[109px] mobile:w-[99px] h-[40px] flex items-center justify-center rounded-[10px]">수정하기</button>
