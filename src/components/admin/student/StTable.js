...
import { useAdStudentQuery } from "../query";
...
const StTable = ({cancleText, cancleText2, keyword, header, updateSelection, deleteMutate}) => {
  ...
  const { studentData, isLoading } = useAdStudentQuery(paramsPage, keyword);
  const [current, setCurrent] = useState(studentData?.data.currentPage + 1);
  ...
  const onChangeAllSelection = (e) => {
    if (e.target.checked) {
      const allCheckedSelection = new Set(studentData?.data.results.map((data) => data.id));
  ...
  const isSelectedAll = () => {
    return selection.size === studentData?.data.results?.length;
  };
  ...
  useEffect(() => {
    ...
    setCurrent(studentData?.data.currentPage + 1)
  },[deleteMutate.isSuccess, studentData?.data])
  ...
  return ( 
    <div className="mt-[12px]">
      ...
      {modifyModal && (
        <BlueModalStPut setState={setModifyModal} data={modifyId}/>
      )}
      {/* 테이블 */}
      ...
      <div className="flex justify-center items-center w-full tablet:gap-[60px] mobile:gap-[30px] text-[12px] mt-[12px]">
        ...
        <div className="items-center text-grayDark font-nanum_400">
          {current} / {studentData?.data.totalPage}
        </div>

        <button onClick={NextPage} disabled={current === studentData?.data.totalPage} className={current === studentData?.data.totalPage ? 'invisible' : ''}>
          <div className="flex justify-center items-center gap-[3px]">
