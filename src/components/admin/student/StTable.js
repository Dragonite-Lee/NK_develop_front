...
const StTable = ({
  ...
  const [current, setCurrent] = useState(studentData?.data.currentPage);
  ...
  const onChangeAllSelection = (e) => {
    if (e.target.checked) {
      const allCheckedSelection = new Set(
        studentData?.data.results.map((data) => data.id)
      );
  ...
  useEffect(() => {
    ...
    setCurrent(studentData?.data.currentPage);
  ...
  if (studentData?.data.results.length == 0)
    return (
  ...
  return (
    <div className="mt-[12px]">
      ...
      <div className="flex justify-center items-center w-full tablet:gap-[60px] mobile:gap-[30px] text-[12px] mt-[12px]">
        ...
        <button
          onClick={NextPage}
          disabled={current === studentData?.data.totalPage}
          className={current === studentData?.data.totalPage ? "invisible" : ""}
        >
          <div className="flex justify-center items-center gap-[3px]">
