...
import { useAdTeacherQuery } from "../adminQuery";
...
const TeTable = ({
  ...
  const [current, setCurrent] = useState(teacherData?.data.currentPage);
  ...
  const onChangeAllSelection = (e) => {
    if (e.target.checked) {
      const allCheckedSelection = new Set(
        teacherData?.data.results.map((data) => data.id)
      );
  ...
  useEffect(() => {
    ...
    setCurrent(teacherData?.data.currentPage);
  ...
  return (
    <div className="mt-[12px]">
      ...
      <div className="flex justify-center items-center w-full tablet:gap-[60px] mobile:gap-[30px] text-[12px] mt-[12px]">
        ...
        <button
          onClick={NextPage}
          disabled={current === teacherData?.data.totalPage}
          className={current === teacherData?.data.totalPage ? "invisible" : ""}
        >
          <div className="flex justify-center items-center gap-[3px]">
