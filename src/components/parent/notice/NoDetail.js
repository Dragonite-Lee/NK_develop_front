...
const NoDetail = ({ID}) => {
  ...
  return (
    <section>
			<Link to="/parent/noticelist" className ="w-full font-paybooc_700 " >
      ...
      <div className="px-[40px] py-[32px] mt-[24px] mobile:p-[24px] desktop:w-[994px] h-fit w-full glassWhite">
        ...
        <Button backgroundColor='#D1D1D6' Color="white" content="이전 공지 보기" link={`/parent/noticelist/:${ID-1}`}/>
        <Button backgroundColor="gray" Color="white" content="목록으로 돌아가기" link="/parent/noticelist/" />
        <Button backgroundColor="#D1D1D6" Color="white" content="다음 공지 보기" link={`/parent/noticelist/:${ID+1}`}/>
      </div>
    </section>
  );
}
