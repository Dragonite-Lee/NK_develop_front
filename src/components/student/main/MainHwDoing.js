...
const StMainHwDoing = () => {
  const { homeworkIdClient, homeworkTitleClient, homeworkTeacherClient, hoemworkTimewatch } = useStudentCurrentHomeworkStore();
  ...
  const formatTime = (time) => {
    ...
    if (getHours !== '00') return `${getHours} 시간 ${getMinutes} 분 ${getSeconds}초`;
    ...
    <section className="glassWhite mt-[24px] py-[24px] desktop:w-[486px] mobile:w-full tablet:px-[32px] mobile:px-[26px]">
      ...
      <div className="mt-[24px] flex items-center justify-between">
        ...
        <Link to={`main/homeworkSt/homeworkDetail/${homeworkIdClient}`}>
          <div className={`bg-homework1 text-white rounded-[10px] w-[130px] h-[36px] flex items-center justify-center`}>
            숙제하러가기
          </div>
        </Link>
      </div>
