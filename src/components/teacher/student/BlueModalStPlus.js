...
import { useTeAllStudentQuery } from "../teacherQuery";
import { postTeClassroomStudent } from "../../../services/api/teacherApi";
...
const BlueModalStPlus = ({ setState, id }) => {
  ...
  const { allStudentData } = useTeAllStudentQuery();

  const postMutate = useMutation({
    mutationFn: () => {
      return postTeClassroomStudent(id, studentPostData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries("/teacher/classroom/student");
      alert("학생이 추가되었습니다.");
      setState(false);
    },
  });
...
export default BlueModalStPlus;
