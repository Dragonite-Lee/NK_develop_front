...
import { useAdAllAdminNoticeQuery } from "../../components/admin/query";
...
const NoticeAd = () => {
  ...
  if (allAdminNoticeData?.data.length == 0) return <div className="font-nanum_700 text-[14px]">등록된 데이터가 존재하지 않습니다.</div>
        ...
        <Header/>
          <main className='desktop:w-[996px] desktop:mx-auto tablet:w-auto tablet:mx-[40px] mobile:mx-[20px] pt-[28px] pb-[58px] mainHeight'>
            <div className="flex items-center justify-between">
              <div className="font-paybooc_700 text-[18px] text-black">전체 공지 ({allAdminNoticeData?.data.length})</div>
            ...
            <div className="mt-[13px] flex tablet:items-center tablet:justify-end mobile:items-end mobile:justify-center tablet:gap-[20px] mobile:gap-[13px] tablet:flex-row mobile:flex-col">
              <div className="flex items-center justify-center gap-[5px] text-[13px] font-nanum_700">
                <div onClick={() => onChangeSelection('STUDENT')} className={`w-[55px] h-[26px] rounded-[10px] flex items-center justify-center ${stToggle ? " bg-main1/30" : " bg-white1"} text-main1 border border-main1 outline outline-1 outline-main1`}>
                  # 학생
                </div>
                <div onClick={() => onChangeSelection('PARENT')} className={`w-[67px] h-[26px] rounded-[10px] flex items-center justify-center ${paToggle ? " bg-main2/30" : " bg-white1"} text-main2 border border-main2 outline outline-1 outline-main2`}>
                  # 학부모
                </div>
                <div onClick={() => onChangeSelection('TEACHER')} className={`w-[67px] h-[26px] rounded-[10px] flex items-center justify-center ${teToggle ? " bg-main3/30" : " bg-white1"} text-main3 border border-main3 outline outline-1 outline-main3`}>
                  # 선생님
                </div>
              </div>
              <div className="relative">
                <input type="text" placeholder="검색어를 검색하세요" value={searchInput} onChange={setSearchInput} className="tablet:w-[300px] mobile:w-[240px] h-[36px] text-[14px] font-nanum_400 pl-[15px] rounded-[10px] bg-whiteTotal drop-shadow-sm" />
