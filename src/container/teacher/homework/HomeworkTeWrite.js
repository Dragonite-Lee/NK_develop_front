...
import { putTeHomework } from "../../../services/api/teacherApi";
...
const HomeworkTeNewWrite = () => {
  ...
  const putMutate = useMutation({
    mutationFn: (data) => {
      return putTeHomework(classnameIdClient, id, data);
    },
    onSuccess: () => {
  ...
  useEffect(() => {
    if (oneHomeworkData) {
      setTitle(oneHomeworkData.data.title)
      setContent(oneHomeworkData.data.description)
