...
const SubmitDetailTe = () => {
  ...
  const { oneHomeworkData } = useTeOneHomeworkQuery(
    classnameIdClient,
    homeworkId
  );
    ...
    <>
      ...
      <div className='min-w-[280px]'>
        ...
        <main className='desktop:w-[996px] desktop:mx-auto tablet:w-auto tablet:mx-[40px] mobile:mx-[20px] pt-[28px] pb-[58px] mainHeight'>
          <Link
            to={`/main/homeworkTe/homeworkDetail/${homeworkDetailStudentData?.data.homeworkId}`}
          >
          ...
          <div className='glassWhite desktop:w-[999px] py-[32px] px-[40px]'>
            <div className='font-nanum_700 text-[13px] flex tablet:flex-row mobile:flex-col items-start justify-between gap-[12px]'>
              <div>
                {homeworkDetailStudentData?.data.studentName} 학생의 숙제 내역
              </div>
              {homeworkDetailStudentData?.data.status == 'COMPLETE' ? (
                <div className='w-[103px] h-[26px] text-homework2 rounded-[10px] flex items-center justify-center border border-homework2 outline outline-1 outline-homework2'>
                  검사 완료 숙제
                </div>
              ) : (
                <div className='w-[81px] h-[26px] text-homework1 rounded-[10px] flex items-center justify-center border border-homework1 outline outline-1 outline-homework1'>
                  검사할 숙제
                </div>
              )}
            </div>
            <div className='text-[13px] flex flex-col item-end justify-center pt-[20px] pb-[16px] gap-[10px] text-grayDark'>
              <div className='flex items-center justify-end font-nanum_400 text-grayDark'>
                {oneHomeworkData?.data.title}
              </div>
            ...
            <div className='py-[28px] border-t-[1px] border-[#f0f0f0]'>
              ...
              <div className='mt-[16px] flex flex-wrap items-center justify-start gap-[16px]'>
                {homeworkDetailStudentData?.data.fileIds.length > 0 ? (
                  homeworkDetailStudentData?.data.fileIds.map((id, index) => (
            ...
            <div className='border-t-[1px] border-[#f0f0f0] pt-[28px] font-nanum_700 text-[15px] text-white '>
              <div className='w-full flex items-center justify-between gap-[10px]'>
                <button
                  ...
                  className='w-full h-[41px] bg-homework1 rounded-[10px]'
                >
                  숙제 반려하기
                </button>
                <button
                  onClick={() => setCompleteModal(true)}
                  className='w-full h-[41px] bg-main1 rounded-[10px]'
                >
                  숙제 승인하기
                </button>
              </div>
